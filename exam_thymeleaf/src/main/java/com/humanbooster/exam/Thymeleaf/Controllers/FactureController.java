...
public class FactureController {
    ...
    public ModelAndView detail(@PathVariable(required = false) Facture facture){
        ...
        ModelAndView mv = new ModelAndView("website/detail");
        mv.addObject("facture", facture);

        double totalHT = facture.getLignes().stream()
                .mapToDouble(ligne -> ligne.getPrixHt().doubleValue() * ligne.getQuantity().doubleValue())
                .sum();

        double totalTTC = facture.getLignes().stream()
                .mapToDouble(ligne -> (ligne.getPrixHt().doubleValue() * ligne.getQuantity().doubleValue()) * (1 + ligne.getTva().getTaux()))
                .sum();

        mv.addObject("totalHT", totalHT);
        mv.addObject("totalTTC", totalTTC);
